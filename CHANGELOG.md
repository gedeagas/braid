...
- Migrated the auto-updater from `electron-updater` to Electron's built-in Squirrel.Mac (`update.electronjs.org`), with release discovery decoupled from download so updates are user-accepted before installing
...
- robust reconnect after long background + connection overlay (#124)
- background push, bare terminal tab, notification & usage fixes (#123)
...
- terminal mobile-fit restore, corner-inset module & home redesign (#121)

### Other

- Harden mobile companion: transport, terminal sizing, driver overlay & pairing UX (#122)
...
- live-terminal hardening, diagnostics, worktree management & PR status (#120)
- Braid mobile companion app + desktop mobile server (#119)
...
- mobile companion server with E2EE pairing (#97)

### Fixed

- grant all webview permissions without dialog prompt (#99)
...
- allow Widevine and prompt on unknown permissions (#98)
...
- multi-agent hook services with per-agent server routing (#96)

### Fixed

- drag-and-drop file paths, terminal re-theming, and Widevine CDM for webviews (#95)
...
- add GitHub integration settings panel (#69)
- add rate limit visualization in chat footer (#68)
