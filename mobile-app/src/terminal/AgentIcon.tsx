...
const CodexIcon = ({ size }: { size: number }) => (
...
const CopilotIcon = ({ size }: { size: number }) => (
  <Svg width={size} height={size} viewBox="0 0 16 16" fill={colors.text}>
...
const PiIcon = ({ size }: { size: number }) => (
  <Svg width={size} height={size} viewBox="0 0 32 32" fill={colors.text}>
...
const BRAND_ICON_MAP: Record<string, (props: { size: number }) => JSX.Element> = {
  claude: ({ size }) => <ClaudeIcon size={size} />,
  codex: ({ size }) => <CodexIcon size={size} />,
  gemini: GeminiIcon,
