// Holds a macOS `caffeinate -i -s` process while agents are working so the
// machine doesn't idle/system-sleep mid-run. Note: `-i -s` does NOT override a
// physically closed MacBook lid (that needs clamshell or pmset) - it keeps the
// Mac awake while the lid is open/idle or running headless on AC power.
