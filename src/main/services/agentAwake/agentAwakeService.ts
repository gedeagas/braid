// Keeps the computer awake while one or more agents are actively "working".
// Combines an Electron powerSaveBlocker with platform assertions (macOS
// caffeinate, Linux systemd-inhibit). Activity-gated: only blocks sleep while a
// status is `working`, observed in the current runtime, and not stale (>2h), and
// only when enabled. Re-asserts on power resume. NOTE: this does not defeat a
// physically closed MacBook lid - it keeps the machine awake while the lid is
// open/idle or running headless. See the macOS/Linux assertion files.
import { powerMonitor, powerSaveBlocker } from 'electron'
import { LinuxLidSleepAssertion } from './linuxLidSleepAssertion'
import { MacosSystemSleepAssertion } from './macosSystemSleepAssertion'

// Mirrors agentHookServer's AgentStatusState; keep this union in sync with that file.
