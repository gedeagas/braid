...
describe('agentProcess entry point', () => {
  ...
  it('dispatches startSession and streams back events', async () => {
    ...
    sendCommand({
      ...
      worktreePath: '/tmp', prompt: 'hello', model: 'claude-sonnet-4-6', thinking: false, extendedContext: false,
  ...
  it('dispatches stopSession without crashing', async () => {
    ...
    sendCommand({
      ...
      worktreePath: '/tmp', prompt: 'hi', model: 'claude-sonnet-4-6', thinking: false, extendedContext: false,
      planMode: false, sessionName: 'Test', settings: defaultSettings
    })
    await waitFor(() =>
