...
describe('canUseTool — allow list auto-approves pre-approved tools', () => {
  ...
  it('auto-allows a tool matched by allow rule — no waiting_input emitted', async () => {
    ...
    await worker.startSession('s1', 'wt-1', 'test', '/tmp', 'hi', 'claude-sonnet-4-6', false, false, false, 'Chat', noBypassSettings)
  ...
  it('tool not in allow list triggers waiting_input when not in bypass mode', async () => {
    ...
    await w.startSession('s1', 'wt-1', 'test', '/tmp', 'hi', 'claude-sonnet-4-6', false, false, false, 'Chat', noBypassSettings)
...
describe('canUseTool — bypassPermissions mode', () => {
  ...
  it('auto-allows any tool when bypassPermissions is true', async () => {
    ...
    await worker.startSession('s1', 'wt-1', 'test', '/tmp', 'hi', 'claude-sonnet-4-6', false, false, false, 'Chat', bypassSettings)
...
describe('AgentWorker', () => {
  ...
  describe('updateSessionName', () => {
    it('updates session name after startSession', async () => {
      mockQuery.mockReturnValue(makeAsyncIterable([]))
      await worker.startSession('s1', 'wt-1', 'test', '/tmp', 'hi', 'claude-sonnet-4-6', false, false, false, 'Original', defaultSettings)
  ...
  describe('startSession', () => {
    it('emits done on successful completion', async () => {
      ...
      await worker.startSession('s1', 'wt-1', 'test', '/tmp', 'hi', 'claude-sonnet-4-6', false, false, false, 'Chat', defaultSettings)
    ...
    it('applies API key from settings', async () => {
      ...
      mockQuery.mockReturnValue(makeAsyncIterable([]))
      await worker.startSession('s1', 'wt-1', 'test', '/tmp', 'hi', 'claude-sonnet-4-6', false, false, false, 'Chat', {
        apiKey: 'sk-test-key',
  ...
  describe('sendMessage', () => {
    it('emits error when no sdkSessionId available', async () => {
      await worker.sendMessage('s1', 'msg', '', '/tmp', 'claude-sonnet-4-6', false, false, 'Chat', defaultSettings)
      const errors = emitted.filter(e => e.type === 'error')
      expect(errors).toHaveLength(1)
    ...
    it('recovers state from params when session map is empty', async () => {
      mockQuery.mockReturnValue(makeAsyncIterable([]))
      await worker.sendMessage('s2', 'msg', 'sdk-abc', '/tmp/proj', 'claude-sonnet-4-6', false, false, 'Recovered', defaultSettings)
    ...
    it('emits done on successful resume', async () => {
      mockQuery.mockReturnValue(makeAsyncIterable([]))
      await worker.sendMessage('s3', 'msg', 'sdk-abc', '/tmp', 'claude-sonnet-4-6', false, false, 'Chat', defaultSettings)
      const doneEvents = emitted.filter(e => e.type === 'done')
      expect(doneEvents).toHaveLength(1)
    })
  })
